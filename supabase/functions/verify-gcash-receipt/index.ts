...
type OcrResult = {
  text: string;
  layoutText?: string;
  nativeLines?: GoogleVisionOcrResult["nativeLines"];
  originalText?: string;
...
function publicReceiptMessage(
  ...
  if (result === "manual_review") {
    if (flags.includes("OCR_UNAVAILABLE")) {
      return "Your receipt is saved. The automatic checking service could not complete the check; the owner will review your payment. Please do not pay again.";
    }
...
async function runOCR(
  ...
  provider: PaymentProvider,
  typedRef: string,
  ocr: typeof googleVisionOcr = googleVisionOcr,
): Promise<OcrResult> {
  if (visionKey) {
    try {
      ...
      return {
        ...
        primaryProvider: "google_vision",
        error: errMsg(e),
        requestMetrics: (e as { requestMetrics?: GoogleVisionRequestMetrics })?.requestMetrics,
      };
    }
  }
  return {
...
Deno.serve(withAdminActivity("verify-gcash-receipt", async (req) => {
  ...
  try {
    ...
    const bookingStartedInstant = (() => {
      const parsed = new Date(String(booking.created_at || booking.createdAt || ""));
      return Number.isNaN(parsed.getTime()) ? null : parsed;
    })();
    const bookingStartedDate = bookingStartedWallClock
      ? bookingStartedWallClock.toISOString().slice(0, 10)
      : null;
    const providerContext: ReceiptVerificationContext = {
      typedReference: typedRef,
      ...
      gotymeRecipientPolicy: settings.gotyme_receipt_recipient_policy === "masked_name_only"
        ? "masked_name_only" : "name_and_account",
      bookingStartedDate,
      paymentWindowMinutes: PAYMENT_WINDOW_MINUTES,
      earlyToleranceMinutes: PAYMENT_EARLY_TOLERANCE_MINUTES,
    };
    const readingSession = createReceiptReadingSession();
    ...
    let gotymeNativeFusion: ReturnType<typeof recoverGotymeNativeFields> | null = null;
    let originalBankReading: Parameters<typeof recoverGotymeNativeFields>[0]["original"] | null = null;
    ...
    let recipientGcashCheck: { clean: boolean; confidence: number } | null = null;
    let recipientReadingAttempted = false;
    let recipientReadingMetrics: GoogleVisionRequestMetrics | undefined;
    let preferredReadingStrategy: string | undefined;
    let readingLayout = provider === "gcash" ? "gcash_unknown" : "unknown";
    let ocrText = "";
    let ocrOriginalText = "";
    ...
    try {
      const ocr = await runOCR(visionKey, imageBase64, provider, typedRef, readingSession.ocr);
      originalOcr = ocr;
      ocrText = ocr.text;
    ...
    if (provider === "gcash" && gcashReceiptHasIncompleteStatus(ocrOriginalText)) {
      flags.push("PAYMENT_STATUS_NOT_COMPLETED");
    }
    if (isBankAdaptiveProvider(provider)) {
      // Preserve adverse status from the original text even when layout
      // reconstruction or a later crop omits that part of the receipt.
      const status = readReceiptTransferStatus(ocrOriginalText);
      if (status.failureStatus) flags.push("TRANSFER_STATUS_INVALID");
      else if (status.pendingStatus) flags.push("TRANSFER_PENDING");
    }
    ...
    if (providerParse?.provider === "gcash") {
      const firstApproval = gcashApprovalConfidence(ocrConfidence, ocrConfidenceSource, gcashOcrEvidence);
      originalGcashCheck = {
        clean: verifyProviderReceipt(providerParse, providerContext).flags.length === 0 &&
          firstApproval.confidence >= 0.9 && ocrConfidenceSource === "native",
        confidence: firstApproval.confidence,
      };
      readingLayout = gcashLayoutFamily(ocrOriginalText);
      ...
      const refinedApproval = gcashApprovalConfidence(
        ocrConfidence, ocrConfidenceSource, gcashOcrEvidence, gcashRecipientRefinement,
      );
      recipientGcashCheck = {
        clean: verifyProviderReceipt(providerParse, providerContext).flags.length === 0 &&
          refinedApproval.confidence >= 0.9 && ocrConfidenceSource === "native",
        confidence: refinedApproval.confidence,
      };
      recipientReadingAttempted = gcashRecipientRefinement?.attempted === true;
      recipientReadingMetrics = gcashRecipientRefinement?.requestMetrics;
      if (!recipientGcashCheck.clean && originalOcr?.provider === "google_vision" &&
          !ocrError && visionKey && !pricingError && flags.length === 0 &&
          readingSession.remainingMs() > 500) {
        // Performance history can choose between tested optical methods only.
        // Missing feedback data must never delay or block a payment check.
        if (readingLayout !== "gcash_unknown") {
          try {
            const preference = await db.rpc("receipt_preferred_reading_strategy", {
              p_layout: readingLayout,
              p_parser_revision: GCASH_VERIFIER_REVISION,
            }).abortSignal(AbortSignal.timeout(1_000));
            if (!preference.error && typeof preference.data?.strategy === "string") {
              preferredReadingStrategy = preference.data.strategy;
            }
          } catch { /* Use the versioned default reading strategy. */ }
        }
        const read: GoogleVisionOcrResult = {
          text: originalOcr.originalText ?? originalOcr.text,
          layoutText: originalOcr.layoutApplied ? originalOcr.text : undefined,
          confidence: originalOcr.confidence,
          confidenceSource: originalOcr.confidenceSource,
          gcashEvidence: originalOcr.gcashEvidence,
          requestMetrics: originalOcr.requestMetrics,
        };
        readingRecovery = await recoverGcashReceipt(bytes, {
          read, parsed: providerParse, refinement: gcashRecipientRefinement,
        }, providerContext, visionKey, {
          preferredStrategy: preferredReadingStrategy,
          ocr: readingSession.ocr,
          deadlineMs: Math.min(12_000, readingSession.remainingMs()),
        });
        if (["recovery_conflict", "recovery_readings_disagree"].includes(readingRecovery.reason) ||
            readingRecovery.reason.startsWith("original_conflict:")) {
          flags.push("OCR_READINGS_DISAGREE");
        }
        if (readingRecovery.audit.readings.some((read) => read.outcome === "error")) {
          flags.push("OCR_REREAD_UNAVAILABLE");
        }
        if (readingRecovery.accepted && readingRecovery.selected) {
          const selected = readingRecovery.selected;
          providerParse = selected.parsed;
          gcashRecipientRefinement = selected.refinement || null;
          gcashOcrEvidence = selected.read.gcashEvidence;
          ocrText = selected.read.gcashEvidence?.layoutText || selected.read.text;
          ocrLayoutApplied = !!selected.read.gcashEvidence?.layoutText;
    ...
    if (providerParse && providerParse.provider !== "gcash" && originalOcr &&
      ...
      originalBankReading = { read: bankRead, parsed: providerParse };
      const firstApproval = bankApprovalConfidence(bankRead, providerParse, providerContext);
      originalBankCheck = {
        clean: verifyProviderReceipt(providerParse, providerContext).flags.length === 0 &&
          firstApproval.confidence >= 0.9 && originalOcr.confidenceSource === "native",
        confidence: firstApproval.confidence,
      };
      ...
      if (providerParse.provider === "gotyme" && !originalBankCheck.clean &&
        !(providerContext.gotymeRecipientPolicy === "masked_name_only" &&
          (firstApproval.fields.recipientName?.confidence ?? 0) >= .9) &&
      ...
      if (!originalBankCheck.clean && originalOcr.provider === "google_vision" &&
        ...
        if (readingLayout !== "unknown") {
          try {
            const preference = await db.rpc("receipt_preferred_provider_reading_strategy", {
              p_provider: providerParse.provider,
              p_destination_provider: providerParse.destinationProvider,
              p_layout: readingLayout,
              p_parser_revision: BANK_VERIFIER_REVISION,
            }).abortSignal(AbortSignal.timeout(1_000));
            if (!preference.error && typeof preference.data?.strategy === "string") {
              preferredReadingStrategy = preference.data.strategy;
            }
          } catch { /* Reading history never blocks a receipt check. */ }
        }
        bankReadingRecovery = await recoverBankReceipt(bytes, {
          read: bankRead, parsed: providerParse,
        }, providerContext, visionKey, {
          preferredStrategy: preferredReadingStrategy,
          ocr: readingSession.ocr,
        ...
        if (["recovery_conflict", "recovery_readings_disagree"].includes(bankReadingRecovery.reason) ||
          bankReadingRecovery.reason.startsWith("original_conflict:")) {
          flags.push("OCR_READINGS_DISAGREE");
        }
        if (bankReadingRecovery.audit.readings.some((read) => read.outcome === "error")) {
          flags.push("OCR_REREAD_UNAVAILABLE");
        }
        if (bankReadingRecovery.accepted && bankReadingRecovery.selected) {
          const selected = bankReadingRecovery.selected;
          providerParse = selected.parsed;
          bankRead = selected.read;
          ocrText = selected.read.layoutText || selected.read.text;
          ocrLayoutApplied = !!selected.read.layoutText;
          ocrConfidence = selected.read.confidence;
          ocrConfidenceSource = selected.read.confidenceSource;
          ocrFallbackReason = null;
        }
    ...
    // Recover a missing principal from actual whole-image reads without
    // discarding stronger original fields. Recipient identity is never filled
    // from settings or upgraded from a different character reading.
    if (providerParse?.provider === "gotyme" && originalBankReading &&
      !bankReadingRecovery?.accepted && !gotymeFieldRecovery?.accepted &&
      !recipientRefinement?.accepted && bankReadingRecovery?.observations?.length) {
      gotymeNativeFusion = recoverGotymeNativeFields({
        original: originalBankReading,
        candidates: bankReadingRecovery.observations,
        context: providerContext,
      });
      if (gotymeNativeFusion.safeToUse && gotymeNativeFusion.applied) {
        providerParse = gotymeNativeFusion.parsed;
        if (gotymeNativeFusion.recoveredFields.includes("amount") &&
          ocrFallbackReason === "google_missing_amount") ocrFallbackReason = null;
      }
      if (gotymeNativeFusion.conservationFlags.some((flag) => /CONFLICT/.test(flag))) {
        flags.push("OCR_READINGS_DISAGREE");
      }
      for (const flag of gotymeNativeFusion.conservationFlags) {
        if (["PAYMENT_STATUS_NOT_COMPLETED", "METHOD_MISMATCH"].includes(flag)) flags.push(flag);
      }
    }
    ...
    if (gotymeFieldRecovery?.audit.readings.some((read) => read.outcome === "error") &&
      ...
      !recipientRefinement?.accepted && !gotymeNativeFusion?.cleanBeforeDuplicateCheck) {
      flags.push("OCR_REREAD_UNAVAILABLE");
    }
    ...
    // Approval uses the provider's observed payment fields. Header/footer text
    // is not a substitute for confidence in a transaction ID or recipient.
    const approval = provider === "gcash"
      ? readingRecovery?.accepted && readingRecovery.selected
        ? readingRecovery.selected.approval
      ...
      : gotymeNativeFusion?.safeToUse && gotymeNativeFusion.applied
      ? gotymeNativeFusion.approval
    ...
    const recipientMatch = providerVerification?.provider === "gcash"
      ...
      : providerVerification?.provider === "gotyme" &&
          providerContext.gotymeRecipientPolicy === "masked_name_only"
      ? ["exact", "masked_compatible"].includes(providerVerification.recipientComparison.name)
      : providerVerification?.provider === "gotyme" ||
    ...
    const verification = {
      ...
      ...(provider === "gotyme" ? { recipientVerificationPolicy: providerContext.gotymeRecipientPolicy } : {}),
    };

    const ocrMetrics = readingSession.metrics();
    const checkedAt = new Date().toISOString();
    const feedbackStrategies = provider === "gcash" ? [
      {
        id: "gcash_original_v1",
        attempted: (originalOcr?.requestMetrics?.calls || ocrMetrics.calls) > 0,
        hardChecksClean: cleanEvidence && originalGcashCheck?.clean === true,
        outcome: originalGcashCheck?.clean ? "clean" : ocrError ? "error" : "uncertain",
        confidence: originalGcashCheck?.confidence ?? 0,
        durationMs: originalOcr?.requestMetrics?.durationMs ?? 0,
        ocrCalls: originalOcr?.requestMetrics?.calls ?? 0,
      },
      ...(recipientReadingAttempted ? [{
        id: "gcash_recipient_pair_v1",
        attempted: true,
        hardChecksClean: cleanEvidence && recipientGcashCheck?.clean === true,
        outcome: recipientGcashCheck?.clean ? "clean" : "uncertain",
        confidence: recipientGcashCheck?.confidence ?? 0,
        durationMs: recipientReadingMetrics?.durationMs ?? 0,
        ocrCalls: recipientReadingMetrics?.calls ?? 0,
      }] : []),
      ...(readingRecovery?.audit.readings || []).map((read) => ({
        id: read.strategy,
        attempted: read.outcome !== "skipped",
        hardChecksClean: cleanEvidence && readingRecovery?.accepted === true && read.outcome === "clean",
        outcome: read.outcome,
        confidence: read.confidence ?? 0,
        durationMs: read.elapsedMs,
        ocrCalls: read.requestMetrics?.calls ?? 0,
      })),
    ] : isBankAdaptiveProvider(provider) ? [
      {
        id: "bank_original_v1",
        attempted: (originalOcr?.requestMetrics?.calls || ocrMetrics.calls) > 0,
        hardChecksClean: cleanEvidence && originalBankCheck?.clean === true,
        outcome: originalBankCheck?.clean ? "clean" : ocrError ? "error" : "uncertain",
        confidence: originalBankCheck?.confidence ?? 0,
        durationMs: originalOcr?.requestMetrics?.durationMs ?? 0,
        ocrCalls: originalOcr?.requestMetrics?.calls ?? 0,
      },
      ...(recipientRefinement?.attempted ? [{
        id: "bank_gotyme_recipient_pair_v1",
        attempted: true,
        hardChecksClean: cleanEvidence && recipientRefinement.accepted && approval.source === "bank_payment_fields",
        outcome: recipientRefinement.accepted ? "clean" : "uncertain",
        confidence: recipientRefinement.accepted ? approvalConfidence : 0,
        durationMs: Math.max(recipientRefinement.nativeRead?.requestMetrics?.durationMs ?? 0,
          recipientRefinement.enlargedRead?.requestMetrics?.durationMs ?? 0),
        ocrCalls: (recipientRefinement.nativeRead?.requestMetrics?.calls ?? 0) +
          (recipientRefinement.enlargedRead?.requestMetrics?.calls ?? 0),
      }] : []),
      ...(bankReadingRecovery?.audit.readings || []).map((read) => ({
        id: read.strategy,
        attempted: true,
        hardChecksClean: cleanEvidence && bankReadingRecovery?.accepted === true && read.outcome === "clean",
        outcome: read.outcome,
        confidence: read.confidence ?? 0,
        durationMs: read.elapsedMs,
        ocrCalls: read.requestMetrics?.calls ?? 0,
    ...
    const extracted = {
      ...
      ...(isDedicatedReceiptProvider(provider)
        ? {
          ...
          ...(gotymeNativeFusion ? { gotymeNativeFusion: {
            strategy: "gotyme_native_fields_v1",
            applied: gotymeNativeFusion.applied,
            cleanBeforeDuplicateCheck: gotymeNativeFusion.cleanBeforeDuplicateCheck,
            recoveredFields: gotymeNativeFusion.recoveredFields,
            usedFields: gotymeNativeFusion.usedFields,
            originalEvidence: gotymeNativeFusion.originalEvidence,
            conservationFlags: gotymeNativeFusion.conservationFlags,
          } } : {}),
          ...(provider !== "gcash" && "fields" in approval ? { paymentFieldEvidence: approval.fields } : {}),
          ...(provider !== "gcash" && "optionalFields" in approval ? { optionalPaymentFieldEvidence: approval.optionalFields } : {}),
          feedback: {
