-- Filter page/action history before pagination; keep service requests separately accessible.
begin;
do $$ begin
...
drop function public.owner_activity_log_list(timestamptz,timestamptz,text,uuid,bigint,integer);
...
begin
  ...
  if p_page is not null and p_page not in ('dash','insights','bookings','deleted','activity','payreview','reports','courts','gamemgr','accounts','remittances','hosts','maintenance','payments') then
  ...
  with matches as (
    select l.* from public.admin_activity_log l where (p_from is null or l.occurred_at>=p_from)
      ...
      and (p_view='all' or (l.source='server_reported' or
        (l.source='client_reported' and l.action='dispatchBookingRescheduleNotifications'))=(p_view='services'))
