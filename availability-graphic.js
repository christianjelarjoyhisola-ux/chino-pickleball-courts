(function (root, factory) {
  ...
  function drawCourtCards(context, summary, layout) {
    ...
    fillRoundRect(context, 72, startY, cardWidth, boardHeight, 26, 'rgba(17,27,36,.96)');
    strokeRoundRect(context, 72, startY, cardWidth, boardHeight, 26, 'rgba(62,121,181,.3)', 2);
    context.fillStyle = '#8eb9e1';
    ...
    timeSlots.forEach((slotWindow, rowIndex) => {
      ...
      context.fillStyle = rowIndex % 2 ? 'rgba(255,255,255,.025)' : 'rgba(142,185,225,.035)';
      ...
      courts.forEach((court, columnIndex) => {
        ...
        const state = slotCellLabel(slot);
        const tones = {
          available: ['rgba(62,121,181,.24)', 'rgba(142,185,225,.72)', '#e8f4ff'],
        ...
        const [fill, stroke, color] = tones[state.tone] || tones.unavailable;
